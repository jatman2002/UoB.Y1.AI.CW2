# adversarialagentTestClasses.py
# ------------------------
...
#     state.getScore()
#           used by adversarialAgents.scoreEvaluationFunction, which is the default
#
import testClasses
import json
...
class AdversarialagentTreeState(object):
    ...
    def generateSuccessor(self, agentIndex, action):
        ...
        return AdversarialagentTreeState(self.problem, successor)
    ...
    def getScore(self):
        ...
        if self.state not in self.problem.evaluation:
            raise Exception(
                'getScore() called on non-terminal state or before maximum '
                'depth achieved.')
...
class AdversarialagentTreeProblem(object):
    def __init__(self, numAgents, startState, winStates, loseStates,
                 successors, evaluation):
        self.startState = AdversarialagentTreeState(self, startState)
...
def parseTreeProblem(testDict):
    ...
    return AdversarialagentTreeProblem(
...
class GradingAgent(Agent):
    def __init__(self, seed, studentAgent, optimalActions, altDepthActions,
...
class PolyAgent(Agent):
    def __init__(self, seed, adversarialAgents, ourPacOptions, depth):
        # prepare our pacman agents
        solutionAgents, alternativeDepthAgents, partialPlyBugAgents = \
            self.construct_our_pacs(adversarialAgents, ourPacOptions)
    ...
    def construct_our_pacs(self, adversarialAgents, keyword_dict):
        pacs_without_stop = [adversarialAgents.StaffAdversarialAgentSearchAgent(
            **keyword_dict) for i in range(3)]
        keyword_dict['keepStop'] = 'True'
        pacs_with_stop = [adversarialAgents.StaffAdversarialAgentSearchAgent(
            **keyword_dict) for i in range(3)]
        keyword_dict['usePartialPlyBug'] = 'True'
        partial_ply_bug_pacs = [
            adversarialAgents.StaffAdversarialAgentSearchAgent(**keyword_dict)]
    ...
    def getTraces(self):
        ...
        return (self.optimalActionLists, self.alternativeDepthLists,
                self.partialPlyBugLists)
...
class PacmanGameTreeTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        adversarialAgents = moduleDict['adversarialAgents']
        studentAgent = getattr(adversarialAgents, self.alg)(depth=self.depth)
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        if self.alg == 'ExpectimaxAgent':
            ...
            ourPacOptions = {}
        pac = PolyAgent(self.seed, adversarialAgents, ourPacOptions, self.depth)
        disp = self.question.getDisplay()
...
class GraphGameTreeTest(testClasses.TestCase):
    ...
    def solveProblem(self, adversarialAgents):
        self.problem.reset()
        studentAgent = getattr(adversarialAgents, self.alg)(depth=self.depth)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        action, generated = self.solveProblem(adversarialAgents)
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.nonTimeoutMinimum = int(
            testDict['nonTimeoutMinimum']) if 'nonTimeoutMinimum' in testDict \
            else None
        ...
        self.maxPoints = sum(
            [len(t) for t in [self.scoreThresholds, self.nonTimeoutThresholds,
                              self.winsThresholds]])
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        agentType = getattr(moduleDict['adversarialAgents'], self.agentName)
        ...
        results = [gradeThreshold(averageScore, self.scoreMinimum,
                                  self.scoreThresholds, "average score"),
                   gradeThreshold(nonTimeouts, self.nonTimeoutMinimum,
                                  self.nonTimeoutThresholds,
                                  "games not timed out"),
                   gradeThreshold(wins, self.winsMinimum, self.winsThresholds,
                                  "wins")]
